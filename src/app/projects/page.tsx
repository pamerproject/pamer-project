...
export default function ProjectsPage() {
  ...
  const fetchProjects = useCallback(
    async (skip: number, take: number): Promise<ProjectItem[]> => {
      const res = await fetch(`/api/projects?skip=${skip}&take=${take}`);
  ...
  const {
    ...
    error,
    loadMore,
  } = usePaginatedFetch<ProjectItem>({ fetchFn: fetchProjects });
            ...
            >
              ...
              <div className="p-4 space-y-3">
                ...
                <div className="flex items-center gap-2 pt-2">
                  <div className="h-6 w-6 rounded-full bg-gradient-to-r from-gray-200 via-gray-100 to-gray-200 dark:from-gray-800 dark:via-gray-700 dark:to-gray-800" />
                  <div className="h-3 w-24 rounded bg-gradient-to-r from-gray-100 via-gray-50 to-gray-100 dark:from-gray-700 dark:via-gray-600 dark:to-gray-700" />
                </div>
              </div>
            </div>
          ))}
  ...
  if (projects.length === 0) {
    return (
      <div className="px-4 md:px-0">
        ...
        <h3 className="text-lg font-bold">{t("project.noProjects")}</h3>
        <p className="mt-1 text-sm text-[var(--muted)]">
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 -mx-2 px-2 scrollbar-none">
            {[1, 2, 3].map((i) => (
              <div key={i} className="w-44 shrink-0 animate-pulse">
                <div className="aspect-video w-full rounded-xl bg-gradient-to-r from-gray-200 via-gray-100 to-gray-200 dark:from-gray-800 dark:via-gray-700 dark:to-gray-800" />
                <div className="mt-2 h-4 w-3/4 rounded bg-gradient-to-r from-gray-200 via-gray-100 to-gray-200 dark:from-gray-800 dark:via-gray-700 dark:to-gray-800" />
                ...
                >
                  <div className="overflow-hidden rounded-xl border border-[var(--card-border)] bg-[var(--card)] transition-all hover:border-[var(--brand)]">
                    {item.image ? (
                      ...
                      <div className="relative flex aspect-video w-full items-center justify-center bg-gradient-to-br from-rose-50 to-rose-100 dark:from-rose-950/30 dark:to-rose-900/20">
                        <svg className="h-8 w-8 text-rose-300 dark:text-rose-700" fill="none" stroke="currentColor" strokeWidth={1} viewBox="0 0 24 24">
