...
export default function ProjectDetailPage() {
  ...
  const loadMoreComments = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/projects/${slug}/comments?skip=${commentSkip + 10}&take=10`);
      if (res.ok) {
  ...
  const fetchComments = useCallback(() => {
    if (!slug) return;
    fetch(`/api/projects/${slug}/comments?skip=0&take=10`, { cache: "no-store" })
  ...
  const isProjectOwner = !!userId && project?.userId === userId;
  ...
  const submitComment = async () => {
    ...
    } catch { setCommentError(t("error.failedToLoad")); } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleStickerComment = async (url: string) => {
    ...
    } catch { setCommentError(t("error.failedToLoad")); }
  };
    ...
    setComments((prev) =>
      prev.map((c) => {
        if (c.id === commentId) {
          ...
          return { ...c, isLiked: newLiked, _count: { ...c._count, likes: c._count.likes + (newLiked ? 1 : -1) } };
        }
        return {
          ...c,
          replies: c.replies.map((r) =>
            r.id === commentId
              ? { ...r, isLiked: !r.isLiked, _count: { ...r._count, likes: r._count.likes + (!r.isLiked ? 1 : -1) } }
              : r
          ),
        };
      })
  ...
  const handlePin = (commentId: string) => {
    ...
    fetch(`/api/comments/${commentId}/pin`, { method: "POST" })
      .then(async (res) => {
        ...
        else setComments(prevCommentsRef.current);
      })
      .catch(() => setComments(prevCommentsRef.current));
  };
  ...
  if (error || !project) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <h2 className="text-lg font-bold">{error || t("error.notFound")}</h2>
  ...
  return (
    <div className="bg-[var(--background)] space-y-1 md:space-y-4">
      ...
      <div className="project-detail-container pt-14 md:pt-0 space-y-0 pb-0 md:space-y-4 md:pb-0">
      {/* ── Mobile Back Button ── */}
      ...
      <div className="hidden md:block">
        <Breadcrumb segments={[
          searchParams.get("ref")?.startsWith("u/") ? { label: t("sidebar.projects"), href: `/${searchParams.get("ref")}` } : { label: t("sidebar.home"), href: "/" },
          { label: t("project.title"), href: "/projects" },
          { label: project.title }
        ]} />
      </div>
      ...
      <div className="card-app rounded-2xl border border-[var(--card-border)] bg-[var(--card)] overflow-hidden">
        ...
        <div className="p-6">
          {/* Header */}
          <div>
            <h1 className="mb-6 text-2xl font-extrabold md:text-3xl">{project.title}</h1>
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                >
                  ...
                  <span className="text-sm font-medium">{project.user.name || project.user.username}</span>
                </Link>
                <span className="text-xs text-[var(--muted)]">
                  {new Date(project.createdAt).toLocaleDateString(lang === "id" ? "id-ID" : "en-US", {
              ...
              <div className="flex shrink-0 gap-2">
                {project.liveUrl && (
                  <a
                    ...
                    className="inline-flex items-center gap-1.5 rounded-lg bg-[var(--brand)] px-4 py-2 text-sm font-medium text-white transition-all hover:bg-[var(--brand-hover)]"
                  >
                    <svg className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                ...
                {project.repoUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-1.5 rounded-lg border border-[var(--card-border)] px-4 py-2 text-sm font-medium transition-all hover:bg-gray-100 dark:hover:bg-gray-800"
                  >
          ...
          {project.description && (
            <div className="mt-6">
              <h2 className="text-sm font-bold uppercase tracking-wider text-[var(--muted)]">{t("communities.description")}</h2>
              <div className="mt-2 text-sm leading-relaxed text-[var(--foreground)] whitespace-pre-wrap break-words">{renderContent(project.description)}</div>
            </div>
          )}

          {/* Stats */}
          <div className="mt-6 flex items-center gap-6 border-t border-[var(--card-border)] pt-4">
            <button
              onClick={handleLikeProject}
              className={`flex items-center gap-1.5 text-sm transition-all ${
                isLiked ? "text-[var(--brand)]" : "text-[var(--muted)] hover:text-[var(--brand)]"
              }`}
            >
              <svg className={`h-4 w-4 ${isLiked ? "fill-current" : "fill-none"}`} stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                <path d="M2 20h2c.55 0 1-.45 1-1v-9c0-.55-.45-1-1-1H2v11zm19.83-7.12c.11-.25.17-.52.17-.8V11c0-1.1-.9-2-2-2h-5.5l.92-4.65c.05-.22.02-.46-.08-.66-.23-.45-.52-.86-.88-1.22L9.46 2.71 8 4.17c-.21.21-.33.48-.33.77v.17l-.95 4.58c-.05.26-.09.52-.09.79v7.42c0 .9.71 1.63 1.6 1.64l7.82.34c.63.03 1.2-.32 1.45-.91l2.5-6.38c.1-.24.16-.5.16-.77z" />
              </svg>
              <span className="font-medium">{likeCount} {t("profile.mantap")}</span>
            </button>
            <div className="flex items-center gap-1.5 text-sm text-[var(--muted)]">
              <svg className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 20.25c4.97 0 9-3.694 9-8.25s-4.03-8.25-9-8.25S3 7.444 3 12c0 2.104.859 4.023 2.273 5.48.432.447.74 1.04.586 1.641a4.483 4.483 0 01-.923 1.785A5.969 5.969 0 006 21c1.282 0 2.47-.402 3.445-1.087.81.22 1.668.337 2.555.337z" />
              </svg>
              <span className="font-medium">{commentCount} {t("project.comments")}</span>
            </div>
            <button
              onClick={handleShareProject}
              className="ml-auto flex items-center gap-1.5 text-sm text-[var(--muted)] hover:text-[var(--brand)] transition-all"
            >
              <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
